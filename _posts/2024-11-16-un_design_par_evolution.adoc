...
[NOTE]
====
Dans le prélude, le premier exemple de permutation des chaînes de caractères présente une complexité de l'ordre de
\(O(n!)\) ce qui la rend comparable à celle du problème du voyageur de commerce.
Cependant, la différence réside dans le fait que, pour la permutation,
l'objectif est de produire toutes les permutations possibles, tandis que, dans le problème du voyageur de commerce,
il s'agit d'identifier la meilleure solution parmi toutes celles qui sont envisageables.
De plus, le problème du voyageur de commerce inclut des contraintes spécifiques, ce qui n'est pas le cas pour la permutation.
Cette distinction est essentielle, car elle classe l'un des algorithmes dans la catégorie des algorithmes exacts et
l'autre dans celle des algorithmes évolutionnaires.
====
...
=== Opt4J
Opt4J est une bibliothèque Java pour l'optimisation basée sur les
